...
public static class Recursion
{
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        // Remember result for potential use later
        remember[s] = ways;
        return ways;
    }
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        else
        {
            for (int i = 0; i < 2; i++)
            {
                string updatedPattern = pattern[..index] + i + pattern[(index + 1)..];
                WildcardBinary(updatedPattern, results);
            }
        }
    }
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        else
        {
            ...
            if (maze.IsValidMove(currPath, x, up))
            {
                ...
                currPath.Remove((x, up));
            }

            // Try Right
            int right = x + 1;
            if (maze.IsValidMove(currPath, right, y))
            {
                currPath.Add((right, y));
                SolveMaze(results, maze, right, y, currPath);
                currPath.Remove((right, y));
            }

            // Try Down
            int down = y + 1;
            if (maze.IsValidMove(currPath, x, down))
            {
                currPath.Add((x, down));
                SolveMaze(results, maze, x, down, currPath);
                currPath.Remove((x, down));
            }

            // Try Left
            int left = x - 1;
            if (maze.IsValidMove(currPath, left, y))
            {
                currPath.Add((left, y));
                SolveMaze(results, maze, left, y, currPath);
                currPath.Remove((left, y));
            }
        }
